...
final class ShouldSameSiteNoneTest extends TestCase {
     ...
     */
    public function testAll($expected, $ua) {
        $this->assertSame($expected, \Uvii\SameSiteNone::isSafe($ua));
    }

    public function allProvider() {
        $rtn = [];
        foreach (self::negativeTestCases as $key => $val) {
            $rtn[$key] = [false, $val];
        }
        foreach (self::postiveTestCases as $key => $val) {
            $rtn[$key] = [true, $val];
        }
        return $rtn;
    }
    ...
    {
        $uaStr = self::negativeTestCases["Chrome 66 Webview"];
        $sameSiteNone = new \Uvii\SameSiteNone($uaStr);
        $this->assertTrue($sameSiteNone->isChromiumVersionAtLeast(51));
    }
}
